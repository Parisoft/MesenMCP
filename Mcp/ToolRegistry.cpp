...
ToolRegistry::ToolRegistry(EmuSession& session) : _session(session)
{
	...
	Register({
		...
		"address + assembly text rows (annotated with source file:line when a .dbg file "
		"is loaded). Supports the console's CPU (6502 for NES, 65816 for "
	...
	Register({
		"set_breakpoint",
		"Set a breakpoint. Three ways to choose the address: (1) plain address (int, "
		"'$C000' or '0xC000'); (2) 'label' - a symbol name from a loaded .dbg file (also "
		"accepted directly in 'address' when it is not hex-prefixed), optionally spanning "
		"the label's size; (3) source-level: 'file' + 'line' (1-based, from a loaded .dbg "
		"file) - breaks on all code generated by that source line, 'end_line' extends the "
		"range. 'access': 'execute' (default), 'read', 'write', or combinations like "
		"'read,write'. 'condition': a debugger expression that must be true to trigger "
		"(e.g. 'A == 0'). Use wait_for_breakpoint or step to observe the hit.",
		ObjectSchema(
			{
				{"address", "integer"},
				{"label", "string"},
			...
			{},
			{
				{"address", "address to break on (int, hex string, or a label name from the .dbg file)"},
				{"label", "symbol name from the loaded .dbg file (preferred over address)"},
				{"file", "source file (name, basename or unique substring) for line breakpoints"},
				{"line", "1-based source line number"},
				{"end_line", "extend a line breakpoint to cover lines up to this one"},
	...
	//--- ca65 .dbg symbol/source tools ---

	Register({
		"load_dbg_file",
		"Load a ca65/cc65 .dbg file (assembled with -g) for the currently loaded ROM. "
		"Imports all labels (code labels registered on PRG-ROM, RAM labels on the CPU "
		"bus - usable in expressions, breakpoint conditions and shown in disassembly) "
		"and builds source-line mappings so set_breakpoint can break on 'file'+'line'. "
		"Load this BEFORE using find_labels / list_source_files / label breakpoints.",
		ObjectSchema({ {"path", "string"}, {"cpu", "string"} }, { "path" },
			{
				{"path", "path to the .dbg file produced when building the ROM"},
				{"cpu", "cpu context (default: main cpu)"}
			}),
		[this](const json& args) { return _session.LoadDbgFile(args); }
	});

	Register({
		"find_labels",
		"Find symbols/labels from the loaded .dbg file and their addresses. Substring "
		"match by default (case-insensitive), exact=true for exact names. Returns "
		"CPU-bus address, PRG offset, defining file:line, size and RAM/code type. "
		"Also usable to discover symbol names before setting label breakpoints.",
		ObjectSchema(
			{
				{"query", "string"},
				{"exact", "boolean"},
			...
			{ "query" },
			{
				{"query", "label name or substring"},
				{"exact", "exact match instead of substring (default false)"},
				{"limit", "max results (default 50)"}
			}),
		[this](const json& args) { return _session.FindLabels(args); }
	});

	Register({
		"list_source_files",
		"List the source files referenced by the loaded .dbg file (with the number of "
		"lines that generated code). Use the file names with set_breakpoint file+line.",
		ObjectSchema({}, {}, {}),
		[this](const json& args) { return _session.ListSourceFiles(args); }
	});
